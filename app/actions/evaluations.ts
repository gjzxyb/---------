...
async function persistEvaluation(
  ...
  const parsedSubmission = evaluationSubmissionSchema.parse(
    parseEvaluationFormData(formData),
  );
  ...
  const questionsById = new Map(
    assignment.task.template.questions.map((question) => [question.id, question]),
  );
  ...
  revalidatePath(`/student/evaluations/${assignment.id}`);
}
